...
from .utils import get_user_region, get_user_rank, get_user_role
...
class ServerSelect(ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        play_role = get_user_role(member)

        player = Player(member.id, member.display_name, region, rank, selected_servers, play_role, self.tag)
        ...
        await interaction.response.send_message(f"{member.display_name} has joined the LF Duo queue!", ephemeral=True) # change back to LFG after event
...
@app_commands.command(name="duo", description="Join the LF Duo queue!") #change back to LFG
async def queue(interaction: discord.Interaction, tag: str = None):
    ...
    if not region and not rank:
        ...
        await interaction.response.send_message(f"You don't have a class role. Please select one in <#{1270091183695331328}>.", ephemeral=True)
        return
...
@app_commands.command(name="leave", description="Leave the LF Duo queue") # change back to LFG
async def leave(interaction: discord.Interaction):
    ...
    if user_id in bot.lfg_queue:
        ...
        await interaction.response.send_message(f"{interaction.user.display_name} has left the LF Duo queue.", ephemeral=True)
    else:
        await interaction.response.send_message(f"{interaction.user.display_name} is not in the LF Duo queue.", ephemeral=True)
