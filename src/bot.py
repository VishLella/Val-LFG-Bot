...
class LFGBot(commands.Bot):
    ...
    async def check_queue(self):
        ...
        for i, info1 in enumerate(players):
            for j, info2 in enumerate(players[i+1:], start=i+1):
                ...
                #if i != j and abs(RANKS[player1.rank.lower()] - RANKS[player2.rank.lower()]) <= 1 and set(player1.servers) & set(player2.servers) and player1.role.lower() != player2.role.lower():
                if i != j and RANKS[player1.rank.lower()] == RANKS[player2.rank.lower()] and set(player1.servers) & set(player2.servers) and player1.role.lower() != player2.role.lower():
                    server_list = list(set(player1.servers) & set(player2.servers))
                    #class_list = [player1.role, player2.role]
    ...
    async def match_players(self, player1, player2, server_list):
        ...
        #LFG-SPAM channel
        # Remove the view parameter and modify the message: #  1291417090640707655
        await self.get_channel(1291417090640707655).send(f"{self.get_user(player1.user_id).mention} and {self.get_user(player2.user_id).mention} have been matched!\n\n**Tags:**\n{player1.tag or ''}\n{player2.tag or ''}\n\n**Servers: {server_list}**")
